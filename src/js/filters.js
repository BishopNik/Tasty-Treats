...
fetchAreas()
	.then(data => {
		...
		elements.countryOptions.insertAdjacentHTML('beforeend', createAreasList(arrCountry));
	})
...
function createAreasList(arr) {
	return arr
		...
		.map(({ name, _id }) => `<li class="list-item js-item" id="${_id}">${name}</li>`)
...
fetchIngred()
	.then(data => {
		...
		elements.ingrOptions.insertAdjacentHTML('beforeend', createIngreedList(arrIngred));
	})
	.catch(error => console.log(error));

function createIngreedList(arr) {
	return arr
		.sort((firstIngr, secondIngr) => firstIngr.name.localeCompare(secondIngr.name))
		.map(({ name, _id }) => `<li class="list-item js-item" id="${_id}">${name}</li>`)
		.join('');
}
// Ingredients api and create markup<<<<<=====
